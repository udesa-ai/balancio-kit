...
"""This module implements an accurate motor model.
Based on: https://github.com/bulletphysics/bullet3/blob/master/examples/pybullet/gym/pybullet_envs/minitaur/envs/motor.py
"""
...
class MotorModel(object):
    ...
    def convert_to_torque(self,
        ...
        """Convert the commands (pwm) to torque, taking into account static and dynamic friction.
          ...
          @param true_motor_velocity: The true motor velocity. The true velocity is used to compute
          back EMF voltage and viscous damping.
          @return actual_torque: The torque that needs to be applied to the motor.
          @return static_friction_torque: Max static friction applied in joint.
          Only applied if velocity is in dead-band range.
        """
        ...
        dynamic_friction_torque = np.invert(vel_in_db) * -1 * np.sign(true_motor_velocity) * np.ones(
            NUM_MOTORS) * DYNAMIC_FRICTION
    ...
    def _convert_to_torque_from_pwm(self, pwm: np.ndarray, true_motor_velocity: np.ndarray) -> np.ndarray:
        """Convert the pwm signal to torque.

          @param pwm: The pulse width modulation.
          @param true_motor_velocity: The true motor velocity at the current moment. It is used to compute
          the back EMF voltage and the viscous damping.
          @return actual_motor_torque: The torque that needs to be applied to the motor.
        """
